...
<div class="car-item card">
    ...
    <div class="p-medium">
      ...
      <h2 class="car-item-title">
          {{ $motor->year }} - {{ $motor->maker?->name ?? 'Unknown Maker' }} {{ $motor->motorModel?->name ?? 'Unknown Model' }}
      </h2>
      <p class="car-item-price">Rp{{ number_format($motor->price, 0, ',', '.') }}</p>
      <hr />
      <p class="m-0">
        <span class="car-item-badge">{{ $motor->motorType?->name ?? 'Type N/A' }}</span>
        <span class="car-item-badge">{{ $motor->fuelType?->name ?? 'Fuel N/A' }}</span>
      </p>
    </div>
</div>